...
# =========================================================
# ECH Tunnel V2 测试版 (双轨隔离并行版)
# 功能：独立的路径、独立的命令 (ech2)、强制附加 -no-ech
# =========================================================
...
download_bin() {
    ...
    if [ ! -f "$BIN_PATH" ]; then
        echo -e "${YELLOW}正在下载 V2 测试版二进制文件...${PLAIN}"
...
load_instance_config() {
    ...
    SERVICE_NAME="ech-tunnel-v2-${INSTANCE_NAME}"

    CFG_IP="104.16.1.1"
    CFG_SERVER=""
    CFG_LISTEN="proxy://0.0.0.0:30007" # 默认端口改一下防冲突
    CFG_TOKEN=""
    CFG_MUX="16" # 默认直接给到 16 路并发
...
create_service() {
    ...
    # 【核心修改】在这里强行加入了 -no-ech 标志
    CMD_ARGS="-l ${CFG_LISTEN} -f ${CFG_SERVER} -ip ${CFG_IP} -n ${CFG_MUX:-16} -no-ech"
    if [ ! -z "$CFG_TOKEN" ]; then
...
[Unit]
Description=ECH Tunnel V2 Client - Instance: ${INSTANCE_NAME}
...
instance_menu() {
    while true; do
        ...
        echo -e "${SKYBLUE}   [V2版] 配置实例: ${YELLOW}${INSTANCE_NAME}${PLAIN}"
        echo -e "${SKYBLUE}====================================${PLAIN}"
        ...
        echo -e " 5. 并发连接数   : ${GREEN}${CFG_MUX:-16}${PLAIN} (多路复用)"
        echo -e "------------------------------------"
        ...
        case "$choice" in
            ...
            5) 
                read -p "请输入并发TCP连接数(建议1-32，默认16): " i
                if [[ "$i" =~ ^[0-9]+$ ]] && [ "$i" -ge 1 ] && [ "$i" -le 64 ]; then
                    CFG_MUX="$i"
                    save_config
                    echo -e "${GREEN}并发连接数已设置。${PLAIN}"
                    sleep 1
                fi
                ;;
            6) start_service ;;
            ...
            9) uninstall_service; if [ $? -eq 0 ]; then return; fi ;;
            0) return ;; 
            *) echo "无效输入"; sleep 1 ;;
        esac
    done
...
list_instances() {
    while true; do
        ...
        for conf in "${files[@]}"; do
            ...
            name="${filename%.*}"
            if systemctl is-active --quiet "ech-tunnel-v2-${name}"; then
        ...
        if [ "$idx" == "0" ]; then
            ...
            real_index=$((idx-1))
            selected_name=$(basename -- "${files[$real_index]}" .conf)
...
main_menu() {
    while true; do
        clear
        echo -e "${SKYBLUE}====================================${PLAIN}"
        echo -e "${SKYBLUE}   ECH Tunnel [V2 测试版] 管理面板${PLAIN}"
        echo -e "${SKYBLUE}====================================${PLAIN}"
        echo -e " 1. 查看 / 管理实例"
        echo -e " 2. 新建实例"
        echo -e " ------------------------"
